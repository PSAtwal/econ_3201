...
knitr::opts_chunk$set(dev = 'pdf', fig.width = 6, fig.height = 4)
...
plot(mpg$cty, mpg$hwy,
     ...
     ylab = 'hwy')
```
    (i) Change the x-axis label using the option `xlab = "City MPG"` and change 
    the y-axis label using the option `ylab = "Highway MPG"`. 
``` {r}
plot(mpg$cty, mpg$hwy,
     xlab = "City MPG",
     ylab = "Highway MPG")
```
    ...
    ## Part (ii) and (iii) answered together below as adding a caption assigns
    the figure no. 1 to it.
    ...
    (iii) Cross reference the figure and add the text "Figure 1 shows the fuel
    efficiency for city driving versus highway driving". 
...
plot(mpg$cty, mpg$hwy, 
     ...
     ylab = "Highway MPG")
```
@fig-cty-hwy shows the fuel efficiency for city driving versus highway driving.
